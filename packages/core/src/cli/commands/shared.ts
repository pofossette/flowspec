...
import { flowSpecSchema, safeParseFlowSpec } from '../../domain/flow-spec.js';
import {
  isMarkdownFlowSpec,
  parseFlowSpecFromMarkdown,
  serializeFlowSpecToMarkdown,
} from '../../domain/flow-spec-md.js';
import { ensureRegistryDir, resolveRegistryDir } from '../../registry/paths.js';
...
export async function syncFromFilesystemSafe(root?: string): Promise<void> {
  try {
    const { syncFromFilesystem } = await import('../../registry/store.js');
