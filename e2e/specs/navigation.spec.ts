...
`;
    ...
    await page.route('**/api/flow-spec?dir=*', async (route) => {
      ...
      if (reqUrl.includes(encodeURIComponent(flowspecDir)) || reqUrl.includes(flowspecDir)) {
        const entries = [
          ...
          { id: secondId, title: secondTitle, path: `${flowspecDir}/${secondId}.md`, rootId: 'root-1' },
        ];
        await route.fulfill({
          status: 200,
    ...
    await expect.poll(async () => new URL(page.url()).searchParams.get('id'), { timeout: 5000 }).toBe(secondId);
  });
});
